...
void disp_calculate(Mat img_1, Mat img_2, Mat gt, String str, String out_dir, int  disp_ratio, float ratio, String disparity_type, String dataset)
{
    ...
    namedWindow("Good Matches_" + str + dataset, cv::WINDOW_AUTOSIZE);
    imshow("Good Matches_" + str + dataset, img_matches);
    imwrite(out_dir + "/Good Matches_" + str + dataset + ".png", img_matches);
    ...
    unsigned long rms = 0, count = 0, max_disp = 0; //para tenerlo con 4 bites
    ...
    imshow("disparity_" + str + dataset + ";    rms:" + to_string(sqrt(rms / count)) + "    bp%: " + to_string(bad[0] / count), disparity);
    imwrite(out_dir + "/disparity_" + str + dataset + ".png", disparity);
    imshow("ground Truth" + dataset,gt);
}
...
int main()
{
    ...
    printf("Stereo correspondence using SIFT, BRISK\n");
    for (int i = 0; i < size; i++) {
        String dataset = dataset_all[i];
        String out_dir = "../Output/" + dataset + "_out_" + disparity_type;
        mkdir(out_dir.c_str(), 07777); //macro que otorga todos los permisos para guardar las imagenes en la carpeta 'Output'
        ...
        //Rezise de la imagen
        resize(img_1, img_1, Size(512, 512));
        resize(img_2, img_2, Size(512, 512));
        resize(gt, gt, Size(512, 512));
    ...
    // Stereo corrspondence Using SIFT and Flann based kd-tree matching cost.
        ...
        disp_calculate(img_1, img_2, gt, "surf", out_dir, disp_ratio, ratio_sift, disparity_type, dataset);

    //Stereo ORB correspondence using Flann based Locality sensitive hashing matching cost
        printf("\nBRISK-%s", dataset.c_str());
        disp_calculate(img_1, img_2, gt, "surf", out_dir, disp_ratio, ratio_orb, disparity_type, dataset);
    }
    waitKey(0);
}
